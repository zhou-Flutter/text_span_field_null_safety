...
import 'package:text_span_field_null_safety/xlog.dart';
...
class TextSpanBuilder {
  ...
  void _deleteLimit(String oldText, String newText, TextSelection oldSelection,
    ...
    for (var i = 0; i < this._customWidgets.length; i++) {
      ...
      if (deleteRange.end <=
                  item.range.start + (deleteRange.end - deleteRange.start) &&
              (deleteRange.start > 0 &&
                  oldText.substring(deleteRange.start - 1, deleteRange.start) !=
                      newText.substring(
                          deleteRange.start - 1, deleteRange.start)) ||
          (oldText.length - newText.length > 1 && !deleted)) {
        // 此时删除的数量为新旧文本的差值
        item.range =
            _updateRange(item.range, -(oldText.length - newText.length));
      } else {
        item.range =
    ...
    if (deleteRange.start > 0 &&
            oldText.substring(deleteRange.start - 1, deleteRange.start) !=
                newText.substring(deleteRange.start - 1, deleteRange.start) ||
        (oldText.length - newText.length > 1 && removeIndex.isEmpty)) {
      finalText = newText;
      finalTextSelection = TextSelection(
  ...
  void customDelete() {
    ...
    if (_lastTextSelection!.extentOffset == 0 ||
        _lastTextSelection!.extentOffset == -1) return;

    if (_lastTextSelection!.extentOffset == 1) {
      return delete(_lastTextSelection!.extentOffset - 1,
          _lastTextSelection!.extentOffset);
    }

    if (_lastTextSelection!.baseOffset != _lastTextSelection!.extentOffset) {
      delete(_lastTextSelection!.baseOffset, _lastTextSelection!.extentOffset);
    } else {
      RegExp rx = RegExp(
          r'[\p{Extended_Pictographic}\u{1F3FB}-\u{1F3FF}\u{1F9B0}-\u{1F9B3}]',
          unicode: true);

      String input = _textEditingController!.text.substring(
          _lastTextSelection!.baseOffset - 2, _lastTextSelection!.extentOffset);
      var isAbc = rx.hasMatch(input);
      if (isAbc) {
        delete(_lastTextSelection!.extentOffset - 2,
            _lastTextSelection!.extentOffset);
      } else {
        delete(_lastTextSelection!.extentOffset - 1,
            _lastTextSelection!.extentOffset);
      }
    }
  }
